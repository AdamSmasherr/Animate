...
def convert_rotation_order_for_objects(objects, new_rotation_mode):
    """
    Converts the rotation order for multiple objects or bones while preserving the visual rotation.
    """
    for obj in objects:
        if obj.type == "ARMATURE" and obj.animation_data and obj.animation_data.action:
            for bone in obj.pose.bones:
                if bone.select:
                    convert_rotation_order(obj, bone, new_rotation_mode, is_bone=True)
        else:
...
class AMP_PT_rotation_mode_recommendation(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not obj:
            return
        element = context.active_pose_bone if (obj.type == "ARMATURE" and context.active_pose_bone) else obj
...
class AMP_OT_bake_to_rotation_mode(bpy.types.Operator):
    ...
    def execute(self, context):
        original_frame = context.scene.frame_current  # <-- Save current frame
        ...
        in_pose_mode = (
            context.active_object and context.active_object.type == "ARMATURE" and context.active_object.mode == "POSE"
        )
