...
from .. import utils  # New: use utility functions for keyframe gathering
...
Hold Shift while clicking to open the options panel."""
...
- Negative values shift keyframes to the left.""",
    ...
    def find_previous_frame(self, layer, frame_number):
        ...
        previous_frames = [getattr(f, "frame_number", getattr(f, "frame", 0)) for f in layer.frames if getattr(f, "frame_number", getattr(f, "frame", 0)) < frame_number]
    ...
    def find_next_frame(self, layer, frame_number):
        ...
        next_frames = [getattr(f, "frame_number", getattr(f, "frame", 0)) for f in layer.frames if getattr(f, "frame_number", getattr(f, "frame", 0)) > frame_number]
        if not next_frames:
            return None
        next_frame_number = min(next_frames)
        return self.find_frame(layer, next_frame_number)

    def shift_gpencil_keyframes(self, gp_frames, shift, context):
        """Shift Grease Pencil frames."""
        unique_layers = set(ref.layer for ref in gp_frames)
        for layer in unique_layers:
            frames_to_shift = [
                getattr(frame, "frame_number", getattr(frame, "frame", 0))
                for frame in layer.frames
                if (shift > 0 and getattr(frame, "frame_number", getattr(frame, "frame", 0)) > self.current_frame)
                or (shift < 0 and getattr(frame, "frame_number", getattr(frame, "frame", 0)) < self.current_frame)
            ]
            ...
            for frame_number in frames_to_shift:
                try:
                    utils.blender_compat.move_grease_pencil_frame(layer, frame_number, frame_number + shift)
                ...
                if self.add_on_slice:
                    ...
                    if source_frame:
                        try:
                            source_fn = getattr(source_frame, "frame_number", getattr(source_frame, "frame", 0))
                            utils.blender_compat.copy_grease_pencil_frame(layer, source_fn, new_slice_frame)
                        except RuntimeError as e:
                            self.report(
                                {"ERROR"},
                                f"Failed to copy frame {source_fn} to {new_slice_frame} in layer '{layer.name}': {e}",
                            )
                    else:
                        self.report(
                            {"WARNING"},
                ...
                if self.add_hold_keyframes:
                    ...
                    if hold_frame_number < context.scene.frame_start or hold_frame_number > context.scene.frame_end:
                        ...
                        if source_frame:
                            try:
                                source_fn = getattr(source_frame, "frame_number", getattr(source_frame, "frame", 0))
                                utils.blender_compat.copy_grease_pencil_frame(layer, source_fn, hold_frame_number)
                            except RuntimeError as e:
                                self.report(
                                    {"ERROR"},
                                    f"Failed to copy frame {source_fn} to {hold_frame_number} in layer '{layer.name}': {e}",
                                )
                        else:
                            self.report(
                                {"WARNING"},
        ...
        for fcu in fcurves:
            ...
            if self.add_hold_keyframes:
                if not any(round(kp.co.x) == round(self.current_frame) for kp in fcu.keyframe_points):
                    fcu.keyframe_points.insert(self.current_frame, current_value, options={"FAST"})

            for kf in fcu.keyframe_points:
                if self.shift_amount > 0 and kf.co.x > self.current_frame:
                    kf.co.x += self.shift_amount
                elif self.shift_amount < 0 and kf.co.x < self.current_frame:
                    kf.co.x += self.shift_amount
