...
Hold SHIFT to launch the TimeWarp options panel."""
    ...
    def poll(cls, context):
        return context.active_object and context.active_object.animation_data  # and context.area.type == "GRAPH_EDITOR"
    ...
    def modal(self, context, event):
        ...
        # **Handle 'SHIFT A' Key Press to Add a Pin at Mouse Location**
        ...
        # **Handle 'ALT A' Key Press to Add a Pin at Mouse Location**
        if event.type == "A" and event.value == "PRESS" and event.alt:
            current_frame = round(context.scene.frame_current)
    ...
    def add_pin(self, context, graph_x, fixed_y):
        ...
        # Allow adding a pin at any frame, including 0
        # Removed restriction to prevent adding at frame 0

        for pin in self.pins:
            if math.isclose(pin.pin_data.position[0], frame, abs_tol=1):
        ...
        # Append the new Pin instance, which references the correct graph position directly
        new_pin = Pin(new_pin_data)
        self.pins.append(new_pin)

        # Debugging output to verify storage in timewarp_pins
    ...
    def drag_pin(self, context, pin, event):
        ...
        graph_x, _ = screen_to_graph(context, event.mouse_region_x, event.mouse_region_y)
        graph_x = self.snap_frame(graph_x)  # Snap to whole frame if enabled
        graph_x = round(graph_x)  # Always snap pins to full frame
    ...
    def save_state(self, context):
        ...
        for pin in self.pins:
            if not pin.pin_data.id_data:
                continue
            pin.pin_data.position = (round(pin.pin_data.position[0]), pin.pin_data.position[1])
    ...
    def store_initial_keyframe_positions(self, context):
        # Instead of using self.scope, use the scene property so that the operator always uses the current scope.
        scope = context.scene.timewarper_settings.scope  # Changed: use scene property
        keyframes = utils.curve.gather_fcurve_keyframes(scope, context)
        initial_co_x = []
        initial_handle_left_x = []
        initial_handle_right_x = []

        for kf in keyframes:
            snapped = self.tw_snap_to_frame and round(kf.co.x) or kf.co.x
            initial_co_x.append(snapped)
            initial_handle_left_x.append(self.tw_snap_to_frame and round(kf.handle_left.x) or kf.handle_left.x)
            initial_handle_right_x.append(self.tw_snap_to_frame and round(kf.handle_right.x) or kf.handle_right.x)
