...
def default_draw_function(layout, context):
    layout.label(text="Fallback", icon="ERROR")
...
def init_sections_and_buttons(section_definitions):
    ...
    if prefs.sections:
        for sec in prefs.sections:
            for btn in sec.buttons:
                if btn.name == "View Options":
                    btn.name = "Anim Curves"
...
SECTION_ICONS = {
    "Tools": "AMP_tools",
    "Toggles": "AMP_toggle", 
    "Utils": "AMP_markers_tools",
    "Keyframes": "AMP_key",
    "View": "AMP_View",
    "Selection": "AMP_selection",
    "Action": "AMP_action",
}
...
def draw_graph_editor_top_bar(self, context):
    ...
    for section in prefs.sections:
        if not section.visible_graph:
            continue

        # Skip Transformator sections from category toggles
        is_transformator = section.name in ["Copy Transforms", "Temp Pivot", "Pin Transforms"]

        if prefs.show_category_buttons and not is_transformator:
            icon_name = SECTION_ICONS.get(section.name, "DOT")
            row.prop(
                section,
                "enable_graph",
                text=section.name,
                toggle=True,
                **get_icon(icon_name) # Use get_icon to support custom icon themes
            )
            
            if section.enable_graph:
                for button in section.buttons:
                ...
                row.separator() # Small gap before next category
        else:
            # Original behavior (or for transformator sections)
            if section.enable_graph:
                for button in section.buttons:
                    if button.enable_graph:
                        button_row = row.row(align=False)
...
def draw_dopesheet_top_bar(self, context):
    ...
    for section in prefs.sections:
        ...
        if prefs.show_category_buttons and not is_transformator:
            ...
            if section.enable_dope:
                for button in section.buttons:
                    if button.enable_dope:
                        button_row = row.row(align=False)
                        button_row.alignment = "LEFT"
                        draw_func = button_draw_funcs.get(button.name, default_draw_function)
                        draw_func(button_row, context)
...
class side_panel_ui(bpy.types.Panel):
    ...
    def draw_header(self, context):
        layout = self.layout
        prefs = bpy.context.preferences.addons[base_package].preferences
        layout.prop(prefs, "refreshing_icons", text="", **get_icon("TOPBAR"), emboss=False)
    ...
    def draw(self, context):
        ...
        row.prop(prefs, "refreshing_icons", text="", icon="FILE_REFRESH")
