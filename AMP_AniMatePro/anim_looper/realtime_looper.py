...
def copy_keyframe_shape(src_kf, dst_kf):
    ...
    src_co = src_kf.co
    dst_co = dst_kf.co
    for handle, src_handle in [(dst_kf.handle_left, src_kf.handle_left),
                               (dst_kf.handle_right, src_kf.handle_right)]:
...
def get_selected_elements():
    ...
    for obj in bpy.context.selected_objects:
        if obj.type == "ARMATURE" and obj.mode == "POSE":
            selected_bones = [b for b in obj.pose.bones if b.select]
...
def deferred_anim_looper_update():
    global _looper_update_scheduled, _is_mutating_internally
    _looper_update_scheduled = False
...
def anim_looper_update_handler(scene):
    ...
    if not realtime_looper_handler_active:
        return
    if not _looper_update_scheduled:
        _looper_update_scheduled = True
        bpy.app.timers.register(deferred_anim_looper_update, first_interval=0.01)
...
class AMP_OT_ToggleRealtimeLooperHandler(bpy.types.Operator):
    ...
    bl_description = ("Ensure continuity between first and last keyframes, "
                      "for keyframe value and handles relative positions")
