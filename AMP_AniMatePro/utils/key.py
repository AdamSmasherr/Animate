...
def update_keyframe_points(context):
    ...
    graph_region = next((r for r in graph_area.regions if r.type == 'WINDOW'), None)
    space = graph_area.spaces.active
    
    snap = getattr(space, "auto_snap", "NONE")
    if hasattr(space, "auto_snap"):
        space.auto_snap = "NONE"

    try:
        with context.temp_override(area=graph_area, region=graph_region):
            bpy.ops.transform.transform()
    except Exception as e:
        print(f"Update keyframe points failed: {e}")

    if hasattr(space, "auto_snap"):
        space.auto_snap = snap
