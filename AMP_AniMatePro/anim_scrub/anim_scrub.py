...
class AMP_OT_ScrubbingTapActionOperator(bpy.types.Operator):
    bl_idname = "anim.amp_scrubbing_action_operator"
    bl_label = "Scrubbing Action Operator"
...
def navigate_through_keyframes(self, context, frame_movement):
    ...
    keyframe_frames = set()
    if pose_mode:
        selected_bones = [bone.name for bone in active_object.pose.bones if bone.select]
...
def navigate_through_frames(self, context, current_frame):
    ...
    if abs(self.cumulative_delta_x) >= 1.0:
        ...
        # Check if we've hit a limit and adjust if necessary
        if new_frame == context.scene.frame_start or new_frame == context.scene.frame_end:
            self.cumulative_delta_x = 0  # Reset to allow immediate direction change
...
class AMP_OT_scrub(bpy.types.Operator):
    ...
    def finish(self, context):
        ...
        if anim_offset.mask_in_use:
            ...
            if context.scene.frame_current < lower_limit:
                context.scene.frame_set(lower_limit)
            elif context.scene.frame_current > upper_limit:
                context.scene.frame_set(upper_limit)
    ...
    def invoke(self, context, event):
        ...
        # self._handle = bpy.types.SpaceView3D.draw_handler_add(
        #     utils.draw_callback_px,
        #     (self, context, event.mouse_x, event.mouse_y),
        #     "WINDOW",
        #     "POST_PIXEL",
        # )
    ...
    def modal(self, context, event):
        ...
        if (
            utils.matches_key_combination(event, prefs.quick_anim_offset_mask_key)
            and anim_offset.quick_anim_offset_in_use
            and not anim_offset.mask_in_use
        ):
            self.drag_started = True
            self.adjusting_quick_anim_offset = True

            # Snapshot the offsets for mask and scrubbing
            self.mask_mouse_offset_x = event.mouse_x
            self.mask_mouse_offset_y = event.mouse_y
            self.initial_mouse_x = event.mouse_x
            self.initial_mouse_y = event.mouse_x
            anim_offset.mask_in_use = True
            anim_offset.reference_frame = context.scene.frame_current
            # self.initial_mouse_x_offset = event.mouse_x
            # bpy.ops.anim.amp_timeline_tools_quick_animoffset_mask()
            bpy.ops.anim.amp_add_anim_offset_mask()
            return {"RUNNING_MODAL"}
        elif (
            anim_offset.mask_in_use
            and utils.matches_key_combination(event, prefs.quick_anim_offset_mask_key)
            or utils.matches_key_combination(event, prefs.quick_anim_offset_blend_key)
        ):
            self.drag_started = True
            self.adjusting_quick_anim_offset = True
        ...
        if scrubbing_key_released and not self.drag_started and not prefs.toggle_for_scrubbing:
            # The spacebar was pressed and released without sufficient dragging
            # if not anim_offset.quick_anim_offset_in_use:
            bpy.ops.anim.amp_scrubbing_action_operator()
            self.finish(context)
            return {"FINISHED"}
        ...
        if self.drag_started and current_mode == "MARKERS":
            if utils.matches_key_combination(event, prefs.remove_marker_keyframe_key):
        ...
        if utils.matches_key_combination(event, prefs.prev_frame_key):
            context.scene.frame_set(context.scene.frame_current - 1)  # Previous frame
            self.pause_dragging = True
            self.store_mouse_pos = False
            self.drag_started = True
        ...
        if utils.matches_key_combination(event, prefs.next_frame_key):
            context.scene.frame_set(context.scene.frame_current + 1)  # Next frame
            self.pause_dragging = True
            self.store_mouse_pos = False
            self.drag_started = True
        ...
        if utils.matches_key_combination(event, prefs.last_frame_key):
            ...
            if context.scene.use_preview_range:
                context.scene.frame_set(context.scene.frame_preview_end)
            else:
