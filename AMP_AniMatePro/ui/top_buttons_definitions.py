...
def AnimBlastButton(layout, context):
    layout.operator(
        "anim.amp_anim_blast",
        text="",
...
def SilhouetteButton(layout, context):
    props = context.scene.amp_silhouette
    layout.prop(
        props,
        "toggle_silhouette",
        text="",
        **get_icon("AMP_silhouette_on" if context.scene.amp_silhouette.toggle_silhouette else "AMP_silhouette_off"),
        emboss=True,
    )


def AnimFlexMoPathsButton(layout, context):
    ...
    row.popover("AMP_FMP_PT_FlexMoPathsPanel", text="")
...
def AnimEditorGreasePencilButton(layout, context):
    if context.area.type in {"GRAPH_EDITOR", "DOPESHEET_EDITOR"}:
        ...
        if context.area.ui_type == "DOPESHEET":
            mode = getattr(context.space_data, "ui_mode", getattr(context.space_data, "mode", ""))
            if mode in ("GPENCIL", "GREASEPENCIL", "GREASE_PENCIL"):
                is_gpencil = True
