...
def match_keymap_item(kmi, keymap_def):
    ...
    for modifier in ["shift", "ctrl", "alt", "oskey", "any"]:
        expected_mod_value = keymap_def.get("modifiers", {}).get(modifier, False)
        if modifier == "any":
            actual_mod_value = bool(getattr(kmi, modifier, False))
        else:
            actual_mod_value = bool(getattr(kmi, modifier + "_ui", False))
...
def toggle_keymaps(keymaps_list, enable):
    ...
    for keymap_def in keymaps_list:
        ...
        if km:
            ...
            if not item_found:
                utils.dprint(f"Default keymap item {keymap_def.get('operator_idname')} not found in {km.name}. Ignoring toggle.")
        else:
            utils.dprint(f"Default keymap {keymap_def['name']} not found. Skipping toggle.")
