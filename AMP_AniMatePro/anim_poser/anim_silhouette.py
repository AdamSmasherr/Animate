...
from .anim_poser import enable_isolate_character, disable_isolate_character
...
def update_toggle_silhouette(self, context):
    scene = context.scene
    ...
    # Apply toggle to all 3D viewports
            ...
            if area.type == "VIEW_3D":
                for space in area.spaces:
                    if space.type == "VIEW_3D":
                        ...
                        if self.toggle_silhouette and existing_info is None:
                            ...
                            if armatures_selected:
                                enable_isolate_character(context, poser_props, enter_pose_mode=False)

                        elif not self.toggle_silhouette and existing_info is not None:
                            shading_info_dict = {
                            ...
                            set_shading_info(shading_info_dict, space)
                            shading_infos.remove(existing_index)

                            if armatures_selected:
                                disable_isolate_character(context, poser_props)
...
class AMP_PG_AnimSilhouetteProps(PropertyGroup):
    ...
    toggle_silhouette: bpy.props.BoolProperty(
        name="Toggle Silhouette",
        description="Toggle silhouette mode globally",
        default=False,
        update=update_toggle_silhouette
    )
