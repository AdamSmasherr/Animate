...
def find_addon_keyconfig(operator_idname, keymaps_to_register, space_type, region_type, properties):
    ...
    utils.dprint(f"No matching keymap found for {operator_idname} in addon keyconfigs.")
    return None
...
def draw_gui_text_lines(context, x, y, toggle_gui_key):
    ...
    if prefs.timeline_gui_toggle and region:
        ...
        lines = [
            ...
            " - Modify Mask Range (No Modifier + Drag)",
            " - Modify Blend Range (Shift + Drag)",
            "",
            "________________________",
            "SCRUBBING HOTKEYS:",
            "________________________",
        ...
        if prefs.current_mode == "KEYFRAMES":
            lines.append(f" - REMOVE KEYFRAME FROM SELECTED ({prefs.remove_marker_keyframe_key})")
...
def update_frame(context, delta_x):
    ...
    context.scene.frame_set(round(new_frame))
...
def add_marker_scrubbing(context):
    ...
    marker.select = True  # Optional: Select the marker after creation
...
def register_script(script, should_register):
    if should_register:
        script.register()
        utils.dprint("Script components registered.")
    else:
        script.unregister()
        utils.dprint("Script components unregistered.")
...
def move_playhead_to_lowest_keyframe(selected_keyframes):
    if selected_keyframes:
        ...
        bpy.context.scene.frame_set(int(lowest_frame))
...
def reset_bar_color():
    ...
    if prefs.original_theme_captured and not anim_offset or not autokeying_is_on:
...
def set_autokeying_theme_colors():
    ...
    if autokeying_is_on:
        ...
        if prefs.autokeying_playhead_color_use:
            if hasattr(theme.dopesheet_editor, "frame_current"):
                theme.dopesheet_editor.frame_current = prefs.autokeying_playhead_color_on
            if hasattr(theme.graph_editor, "frame_current"):
                theme.graph_editor.frame_current = prefs.autokeying_playhead_color_on
            if hasattr(theme.nla_editor, "frame_current"):
                theme.nla_editor.frame_current = prefs.autokeying_playhead_color_on
        else:
            if hasattr(theme.dopesheet_editor, "frame_current") and prefs.original_dopesheet_frame_current:
                theme.dopesheet_editor.frame_current = json.loads(prefs.original_dopesheet_frame_current)
            if hasattr(theme.graph_editor, "frame_current") and prefs.original_graph_frame_current:
                theme.graph_editor.frame_current = json.loads(prefs.original_graph_frame_current)
            if hasattr(theme.nla_editor, "frame_current") and prefs.original_nla_frame_current:
