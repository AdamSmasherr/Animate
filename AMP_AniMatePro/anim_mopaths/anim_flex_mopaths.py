...
def update_visibility(context):
    ...
    try:
        props = context.scene.mp_props
        temp_collection = bpy.data.collections.get("MotionPaths_CustomOrigins")
            ...
            if elem.item_type == "OBJECT":
                try:
                    if not elem.object_ref:
                        continue
                    new_name = elem.object_ref.name
                    if elem.name == new_name:
                        continue
                except ReferenceError:
                    continue

                # Rename empty if it exists
                try:
                    if elem.empty_ref:
                        elem.empty_ref.name = f"AMP_FMP_CustomOrigin_{new_name}"
                except ReferenceError:
                    pass
                
                # Rename sub-collection if it exists
                try:
                    if elem.collection_name and elem.collection_name in bpy.data.collections:
                        old_coll_name = elem.collection_name
                        new_coll_name = f"AMP_FMP_CustomOrigin_{new_name}_Collection"
                        coll = bpy.data.collections[old_coll_name]
                        coll.name = new_coll_name
                        elem.collection_name = coll.name
                except ReferenceError:
                    pass
...
def depsgraph_update_handler(scene):
    ...
    if not _flex_mopaths_update_scheduled:
        ...
        bpy.app.timers.register(deferred_flex_mopaths_update, first_interval=0.01)
...
class AMP_FMP_OT_Remove_Index_Element(Operator):
    ...
    def execute(self, context):
        ...
        try:
            props = context.scene.mp_props
            if not (0 <= self.index < len(props.elements)):
            ...
            try:
                if elem.empty_ref:
                    ...
                    if sub_collection_name and sub_collection_name in bpy.data.collections:
                        sub_collection = bpy.data.collections[sub_collection_name]
                        if empty_name in sub_collection.objects:
                            sub_collection.objects.unlink(empty)
                        if empty_name in bpy.data.objects:
                            bpy.data.objects.remove(empty, do_unlink=True)
                        bpy.data.collections.remove(sub_collection, do_unlink=True)
            ...
            restore_selection_and_mode(context, mode, sel_objs, active_obj, sel_bones, active_bone)
            update_visibility(context)
            self.report({"INFO"}, f"Removed element '{elem_name}'")
            return {"FINISHED"}
        finally:
            lock_object_mode(was_locked)
        ...
        if 0 <= self.index < len(props.elements):
            ...
            try:
                if elem.item_type == "OBJECT" and elem.object_ref:
                    ...
                    self.report({"WARNING"}, "Invalid or deleted element reference")
                    return {"CANCELLED"}
            except ReferenceError:
                self.report({"WARNING"}, "Referenced object or bone has been deleted")
                return {"CANCELLED"}
    ...
    def execute(self, context):
        ...
        if 0 <= self.index < len(props.elements):
            ...
            try:
                ...
                self.report({"WARNING"}, "Referenced empty object has been deleted")
                return {"CANCELLED"}
        else:
            self.report({"WARNING"}, "Invalid index")
            return {"CANCELLED"}
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        if self.layout_type in {"DEFAULT", "COMPACT"}:
            ...
            if item.item_type == "OBJECT":
                try:
                    if item.object_ref:
                        display_name = item.object_ref.name
                except ReferenceError:
                    pass
            row.label(text=display_name, icon=icon_type)
            row.prop(item, "always_show", text="", icon="PINNED" if item.always_show else "UNPINNED", toggle=True)


class AMP_FMP_PT_FlexMoPathsBase(Panel):
...
class AMP_FMP_PT_FlexMoPathsGraph(AMP_FMP_PT_FlexMoPathsBase, Panel):
    bl_label = "Flex MoPaths"
    bl_idname = "AMP_FMP_PT_FlexMoPathsGraph"
...
class AMP_FMP_PT_FlexMoPathsDope(AMP_FMP_PT_FlexMoPathsBase, Panel):
    bl_label = "Flex MoPaths"
    bl_idname = "AMP_FMP_PT_FlexMoPathsDope"
...
class AMP_FMP_PT_FlexMoPathsPanel(AMP_FMP_PT_FlexMoPathsBase, Panel):
    bl_label = "Flex MoPaths"
    bl_idname = "AMP_FMP_PT_FlexMoPathsPanel"
    bl_space_type = "VIEW_3D"
    bl_region_type = "WINDOW"


class AMP_FMP_PT_FlexMoPathsView(AMP_FMP_PT_FlexMoPathsBase, Panel):
    bl_label = "Flex MoPaths"
    bl_idname = "AMP_FMP_PT_FlexMoPathsView"
    bl_space_type = "VIEW_3D"
    bl_region_type = "UI"
...
classes = (
    ...
    AMP_FMP_UL_Elements,
    # AMP_FMP_PT_FlexMoPathsBase,
    AMP_FMP_PT_FlexMoPathsPanel,
    AMP_FMP_PT_FlexMoPathsGraph,
    AMP_FMP_PT_FlexMoPathsDope,
    AMP_FMP_PT_FlexMoPathsView,
)
