...
def anim_sculpt_brush(
    ...
    for fcurve in fcurves:
        ...
        # Replace per-fcurve offset lookup with the passed nla_offset
        keyframe_times = np.array([keyframe.co[0] + nla_offset for keyframe in keyframes])
    ...
    def snap_keyframes_to_frames(self, context):
        ...
        for fcurve in fcurves:
            for keyframe in fcurve.keyframe_points:
                keyframe.co[0] = round(keyframe.co[0] + self.nla_offset) - self.nla_offset
        for fcurve in fcurves:
            fcurve.update()
        context.area.tag_redraw()
    ...
    def invoke(self, context, event):
        ...
        if self.cancel_sculpt:
            if wm.anim_sculpt_running:
                self.report({"INFO"}, "Anim sculpt canceled")
                self.cancel(context)
                wm.anim_sculpt_running = False
                return {"CANCELLED"}
            else:
                self.report({"INFO"}, "Anim sculpt is not running")
                return {"CANCELLED"}
        ...
        if context.area.type == "GRAPH_EDITOR":
            # Adjust mouse position relative to the Graph Editor's viewport
            ...
            # NEW OFFSET CALCULATION: use scene's is_nla_tweakmode flag and active_nla_strip
            if context.scene.is_nla_tweakmode:
                active_strip = bpy.context.active_nla_strip
                if active_strip:
                    self.nla_offset = active_strip.frame_start_ui
                else:
                    self.nla_offset = 0
            else:
                self.nla_offset = get_nla_strip_offset(context.object)

            self.report({"INFO"}, "Anim sculpt started")
            return {"RUNNING_MODAL"}
        else:
            self.report({"WARNING"}, "Active area is not a Graph Editor")
            wm.anim_sculpt_running = False
            return {"CANCELLED"}
...
def draw_anim_sculpt_options(layout, context):
    ...
    brushes_col = layout.column(align=True)
