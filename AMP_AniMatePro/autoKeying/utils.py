...
def register():
    ...
    for area_name, space_type in SPACE_MAPPING.items():
        if area_name not in draw_handler_dict:
            handler = space_type.draw_handler_add(draw_frame, (), 'WINDOW', 'POST_PIXEL')
            draw_handler_dict[area_name] = handler
...
def unregister():
    global draw_handler_text

    if draw_handler_text is not None:
        try:
            bpy.types.SpaceView3D.draw_handler_remove(draw_handler_text, "WINDOW")
        except ValueError:
            pass
    ...
    for area_name, handler in list(draw_handler_dict.items()):
        space_type = SPACE_MAPPING.get(area_name)
        if space_type is not None:
            try:
                space_type.draw_handler_remove(handler, 'WINDOW')
            except ValueError:
                pass
    draw_handler_dict.clear()
