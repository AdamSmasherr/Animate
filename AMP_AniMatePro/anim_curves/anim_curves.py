...
class AMP_OT_cleanup_keyframes_from_locked_transforms(bpy.types.Operator):
    ...
    def cleanup_fcurves_general(self, obj):
        bones = obj.pose.bones if self.affect_scope == "ALL" else [bone for bone in obj.pose.bones if bone.select]
        for bone in bones:
    ...
    def delete_fcurve(self, obj, data_path, index):
        fcurves = (
            utils.curve.get_active_fcurves_obj(obj)
...
class AMP_OT_select_fcurves(bpy.types.Operator):
    ...
    bl_options = {"REGISTER"}
    bl_description = """Toggle F-Curves selection or visibility based on type (Translation, Rotation, Scale, Constraints, Others, All)
    ...
    # New properties for modifier keys
    ...
    def execute(self, context):
        ...
        # Expand curve groups if preference is set
        if prefs.expand_curve_groups:
            bpy.ops.anim.channels_expand()

        # Determine selected objects or bones based on mode
        if context.mode == "OBJECT":
            objects = context.selected_objects
            bones = []
        elif context.mode == "POSE":
        ...
        # Reset cycling index if action type changed
        ...
        # Apply transformation if any keyframe is selected and preference is set
        if self.transform_if_selected and utils.curve.has_selected_keyframes(context):
            print("Transforming")
            self.apply_transform(context)
            return {"FINISHED"}
        else:
            # Handle F-Curve selection based on action type
            self.handle_fcurves_selection(objects, bones, context)
            # Deselect all keyframes in editors
            utils.curve.deselect_all_keyframes_in_editors(context)

            # Zoom to selected curves if preference is set
    ...
    def handle_fcurves_selection(self, objects, bones, context):
        """Select or toggle visibility of F-Curves based on action type and extra options."""
        # Early exit if no objects or bones are selected
        if not objects and not bones:
            return

        # Initialize list to collect relevant F-Curves
        all_fcurves = []

        # Reveal all F-Curves in the Graph Editor if active
        if context.area.type == "GRAPH_EDITOR":
            bpy.ops.graph.reveal()

        # Handle deselection of all F-Curves if required
        if self.extra_options == "DESELECT_ALL":
            self.deselect_all_fcurves(context.visible_fcurves)

        # Get the matching criteria based on action type
        match_criteria = self.get_matching_criteria()

        # Collect F-Curves based on match criteria
        for obj in objects:
            if obj.animation_data and obj.animation_data.action:
                for fc in utils.curve.get_active_fcurves_obj(obj):
                    path = fc.data_path
                    if self.action_type == "ALL":
                        all_fcurves.append(fc)
                    else:
                        if match_criteria(path):
                            all_fcurves.append(fc)

        # Handle bones if in POSE mode
        if bones:
            for bone in bones:
                obj = bone.id_data
                if obj.animation_data and obj.animation_data.action:
    ...
    def cycle_deselect_fcurves(self, fcurves):
        """Cycle through deselecting F-Curves one by one."""
        prefs = bpy.context.preferences.addons[base_package].preferences
        # Clear all selections first
        for fc in fcurves:
            fc.select = False

        if getattr(prefs, "deselect_state_index", -1) == -1:
            # Initial state, select all F-Curves
            for fc in fcurves:
                fc.select = True
            prefs.deselect_state_index = 0  # Prepare for individual selection
        elif 0 <= prefs.deselect_state_index < len(fcurves) and self.cycle:
            # Cycle through F-Curves one by one
            fcurves[prefs.deselect_state_index].select = True
            prefs.deselect_state_index += 1
        else:
            # After cycling to the last F-Curve, select all again and reset
            for fc in fcurves:
                fc.select = True
            prefs.deselect_state_index = 0  # Reset cycle
...
class AMP_OT_JumpToKeyframe(bpy.types.Operator):
    ...
    bl_options = {"REGISTER", "UNDO"}
    bl_description = """Jump to the next or previous keyframe for objects, bones in Pose Mode, and Grease Pencil.
Select the keyframes after jumping to them"""

    # Usage:
    # bpy.ops.anim.amp_jump_to_keyframe(direction='NEXT')
    # bpy.ops.anim.amp_jump_to_keyframe(direction='PREVIOUS')
    # bpy.ops.anim.amp_jump_to_keyframe(direction='NEXT', select_keyframes=True)
    ...
    direction: EnumProperty(
        ...
        default="NEXT",
        description="Direction to jump to the next or previous keyframe",
    )
    ...
    def execute(self, context):
        ...
        keyframes = utils.curve.find_keyframes(context)
        if not keyframes:
        ...
        if self.direction == "NEXT":
            # Find the first keyframe after the current frame
            future_frames = [frame for frame in keyframes_sorted if frame > current_frame]
            if future_frames:
                target_frame = future_frames[0]
            else:
                # Check if cycle-aware keying is enabled
                if bpy.context.scene.tool_settings.use_keyframe_cycle_aware:
                    target_frame = keyframes_sorted[0]  # Jump to the first keyframe
        elif self.direction == "PREVIOUS":
            # Find the last keyframe before the current frame
            past_frames = [frame for frame in keyframes_sorted if frame < current_frame]
            if past_frames:
                target_frame = past_frames[-1]
            else:
                # Check if cycle-aware keying is enabled
                if bpy.context.scene.tool_settings.use_keyframe_cycle_aware:
                    target_frame = keyframes_sorted[-1]  # Jump to the last keyframe

        # if select keyframes is enabled, select the keuyframes (graph editor or dopesheet), or frames in grease pencil
        if self.select_keyframes:
            utils.curve.select_keyframe_in_editors(target_frame, context)

        if target_frame is not None:
            context.scene.frame_set(int(round(target_frame)))
            return {"FINISHED"}
        else:
            self.report({"INFO"}, f"No {self.direction.lower()} keyframe found.")
            return {"CANCELLED"}
...
class AMP_OT_select_keyframes_incurrent_frame(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if obj.animation_data and obj.animation_data.action:
            utils.curve.select_keyframe_in_editors(frame, context)
        ...
        if self.scene_range_to_action:
            ...
            if action:
                if action.use_frame_range:
                    ...
                    for fcurve in utils.curve.get_active_fcurves_obj(context.active_object):
                        for keyframe in fcurve.keyframe_points:
...
def AnimCurvesFrameSelectedButton(layout, text="Frame"):
    has_selected_keyframes = utils.curve.has_selected_keyframes(bpy.context)
    layout.operator(
...
def AnimCurvesSoloButton(layout, text="Solo"):
    ...
    has_selected_keyframes = utils.curve.has_selected_keyframes(context)
...
class AMP_OT_FrameEditors(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        return context.area.type in {"GRAPH_EDITOR", "DOPESHEET_EDITOR", "TIMELINE"}

    def execute(self, context):
        has_selected_keyframes = utils.curve.has_selected_keyframes(context)
        editor = getattr(bpy.ops, "graph") if bpy.context.area.type == "GRAPH_EDITOR" else getattr(bpy.ops, "action")

        self.zoom_in_out(editor, has_selected_keyframes)

        return {"FINISHED"}

    def zoom_in_out(self, editor, has_selected_keyframes):
...
class AMP_OT_view_anim_curves_loc(bpy.types.Operator):
    ...
    bl_label = "Isoalte Location Curves"
    bl_description = (
...
class AMP_OT_view_anim_curves_rot(bpy.types.Operator):
    ...
    bl_label = "Isoalte Rotation Curves"
    bl_description = (
